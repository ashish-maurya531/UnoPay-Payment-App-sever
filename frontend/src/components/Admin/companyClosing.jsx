...
//         </Col>
//         <Col span={24}>
//           <Card title="Manual Monthly Closing">
//             <Button.Group>
//               {['monthly'].map(type => (
//                 <Button
//                   key={type}
...
////////////////////////////////////////
// import { useState, useEffect } from 'react';
// import { Table, notification, Row, Col, Button, Statistic, Tabs, DatePicker, Card ,Input} from 'antd';
// import axios from 'axios';
...
//     try {
//       setLoading(true);
//       const response = await axios.post(`${Src}/api/auth/check-distribute/monthly`, {
...
//               <Text>{`Rs. ${monthlyData.monthlyIncome}`}</Text>
//             </Typography>
//           </Col>
//           <Col span={8}>
//             <Typography>
//               <Text strong>Start Date: </Text>
//               <Text>{monthlyData.startOfMonth}</Text>
//             </Typography>
//           </Col>
//           <Col span={8}>
//             <Typography>
//               <Text strong>End Date: </Text>
//               <Text>{monthlyData.endOfMonth}</Text>
//             </Typography>
//           </Col>
//         </Row>
...
export default function DistributionAndClosing() {
  ...
  const handleManualClosing = async (type) => {
    try {
      setLoading(true);
      await axios.post(`${Src}/api/auth/check-distribute/${type}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
      notification.success({
        message: `${type.charAt(0).toUpperCase() + type.slice(1)} Closing Successful`,
  ...
  const handleRunCustomMonthlyDistribution = async () => {
    try {
      setLoading(true);
      await axios.post(`${Src}/api/auth/check-distribute/monthly`, 
        { custom_monthly_amount_distribution: customMonthlyAmount }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
      notification.success({
        message: 'Monthly Distribution Successful',
        description: 'Custom monthly distribution completed successfully',
      });
      await fetchClosingData();
    } catch (error) {
      notification.error({
